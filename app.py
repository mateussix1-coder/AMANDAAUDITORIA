...
_original_progress = st.progress


def _normalize_options(options):
    return [ui(option) if isinstance(option, str) else option for option in options]


def markdown_fixed(body, *args, **kwargs):
    return _original_markdown(ui(body), *args, **kwargs)


def radio_fixed(label, options, *args, **kwargs):
    return _original_radio(ui(label), _normalize_options(options), *args, **kwargs)


def selectbox_fixed(label, options, *args, **kwargs):
    return _original_selectbox(ui(label), _normalize_options(options), *args, **kwargs)


def text_input_fixed(label, *args, **kwargs):
    if "placeholder" in kwargs:
        kwargs["placeholder"] = ui(kwargs["placeholder"])
    return _original_text_input(ui(label), *args, **kwargs)


def number_input_fixed(label, *args, **kwargs):
    return _original_number_input(ui(label), *args, **kwargs)


def file_uploader_fixed(label, *args, **kwargs):
    return _original_file_uploader(ui(label), *args, **kwargs)


def expander_fixed(label, *args, **kwargs):
    return _original_expander(ui(label), *args, **kwargs)
...
class _ProgressProxy:
    ...
    def progress(self, value, text=None):
        if text is not None:
            return self._obj.progress(value, text=ui(text))
        return self._obj.progress(value)

    def __getattr__(self, name):
        return getattr(self._obj, name)


def progress_fixed(value=0, text=None):
    if text is not None:
        return _ProgressProxy(_original_progress(value, text=ui(text)))
    return _ProgressProxy(_original_progress(value))


st.markdown = markdown_fixed
st.radio = radio_fixed
st.selectbox = selectbox_fixed
st.text_input = text_input_fixed
st.number_input = number_input_fixed
st.file_uploader = file_uploader_fixed
st.expander = expander_fixed
st.progress = progress_fixed
...
def salvar_upload_pdf(uploaded_file, prefixo):
    ...
    caminho = temp_dir / f"{prefixo}_{get_upload_name(uploaded_file)}"
    ...
    with open(caminho, "wb") as arquivo_temp:
...
def set_audit_debug(registros_a=None, registros_b=None, warn_a=None, warn_b=None, caminho_a=None, caminho_b=None):
...
def render_chart(df):
    with st.container(border=True):
        ...
        fig.update_layout(
            ...
            showlegend=False,
            font=dict(family="Inter", color="#202236"),
...
if page in ["Dashboard", "Nova Auditoria"]:
    ...
    if st.session_state.processing:
        ...
        try:
            ...
            set_audit_debug(caminho_a=caminho_a, caminho_b=caminho_b)
            ...
            if not warn_a and not warn_b:
                warn_a = [mensagem]
                warn_b = [mensagem]
            set_audit_debug(warn_a=warn_a, warn_b=warn_b, caminho_a=caminho_a, caminho_b=caminho_b)
            set_audit_error(
