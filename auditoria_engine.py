...
def _extrair_atua_linha_unica(caminho_pdf) -> Dict[str, Dict[str, Any]]:
    registros = {}
...
def _extrair_atua_multilinha(caminho_pdf) -> Dict[str, Dict[str, Any]]:
    registros = {}
    bloco_atual = None
...
def extrair_atua_por_blocos(caminho_pdf) -> Dict[str, Dict[str, Any]]:
    registros = _extrair_atua_linha_unica(caminho_pdf)
    if registros:
        return registros
    return _extrair_atua_multilinha(caminho_pdf)


def _extrair_gw_linha_unica(caminho_pdf) -> Dict[str, Dict[str, Any]]:
    registros = {}
    for page_num, linha in _extrair_linhas_pdfplumber(caminho_pdf):
...
def _extrair_gw_multilinha(caminho_pdf) -> Dict[str, Dict[str, Any]]:
    registros = {}
    bloco_atual = None
...
def extrair_gw_por_blocos(caminho_pdf) -> Dict[str, Dict[str, Any]]:
    registros = _extrair_gw_linha_unica(caminho_pdf)
    if registros:
        return registros
    return _extrair_gw_multilinha(caminho_pdf)
...
def comparar(df_a: pd.DataFrame, df_b: pd.DataFrame, tolerancia: Decimal) -> pd.DataFrame:
    ...
    for cte in sorted(set_a | set_b, key=lambda x: int(x)):
        ...
        calc_ea = ea if ea is not None else Decimal("0.00")
        calc_ma = ma if ma is not None else Decimal("0.00")
        calc_eb = eb if eb is not None else Decimal("0.00")
        calc_mb = mb if mb is not None else Decimal("0.00")
        de = (calc_ea - calc_eb).quantize(CENTAVOS, rounding=ROUND_HALF_UP)
        dm = (calc_ma - calc_mb).quantize(CENTAVOS, rounding=ROUND_HALF_UP)
